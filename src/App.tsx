    ...
    <div className={`app-shell ${ui.navigationOpen ? "nav-open" : ""} ${ui.explanationOpen ? "explanation-open" : "explanation-closed"}`} data-testid="guided-review">
      ...
      <header className="topbar">
        ...
        <div className="topbar-context" aria-label="Repository context">
          <span>opendiff</span><span className="context-slash">/</span><span>{review.git.branch}</span>
...
function loadErrorCopy(error: unknown): { title: string; body: string; detail?: string } {
  if (error instanceof ReviewLoadError) {
    if (error.kind === "missing-review") return { title: "No OpenDiff review was found", body: "Ask the coding agent to generate .opendiff/review.json, then reload this page." };
    ...
    return { title: "The local renderer is unavailable", body: "Start OpenDiff from the repository with opendiff review, then try again.", detail: error.detail };
  }
  ...
  return (
    <main className="load-state load-error" role="alert" data-testid="load-error">
      ...
      <div className="load-state-actions">
        ...
        <span>Generate data with <code>opendiff render</code></span>
      </div>
    </main>
  );
}
